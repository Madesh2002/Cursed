...
async function getStalkerConfig() {
    // Cache the config for 5 minutes to avoid excessive reads
    ...
    // Fallback to default config if Firebase fetch fails or doc doesn't exist
    return {
        host: 'tv.saartv.cc',
...
async function auth(config, token) {
...
async function getStreamURL(config, id, token) {
...
async function genToken(config) {
...
async function convertJsonToM3U(config, channels, profile, account_info, origin, providedToken) {
    let m3u = [
        '#EXTM3U',
...
async function startServer() {
  ...
  const handlePlaylist = async (req, res) => {
    const providedToken = req.params.token;
    const origin = `${req.protocol}://${req.get('host')}`;

    if (providedToken === 'src' || providedToken === 'lib' || providedToken === 'node_modules' || providedToken === 'assets') {
  ...
  app.get('/:token/:id.m3u8', async (req, res) => {
    ...
    try {
        ...
        const stream = await getStreamURL(config, id, token);
        if (!stream) {
