...
async function test() {
    try {
        ...
        const token = tokenData.js.token;
        h["Authorization"] = "Bearer " + token;
        const res = await fetch("http://tv.saartv.cc/stalker_portal/server/load.php?type=itv&action=get_all_channels&JsHttpRequest=1-xml", {headers: h});
